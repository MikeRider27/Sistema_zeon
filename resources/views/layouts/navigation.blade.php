<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @if (Auth::user()->isAdmin() || Auth::user()->isMesero() || Auth::user()->isCaja())
                    ...
                    @if (Auth::user()->isAdmin() || Auth::user()->isCocina())
                        <x-nav-link :href="route('cocina.index')" :active="request()->routeIs('cocina.*')">
                            {{ __('Cocina') }}
                        </x-nav-link>
                    @endif

                    @if (Auth::user()->isAdmin() || Auth::user()->isCaja() || Auth::user()->isMesero())
                        <x-nav-link :href="route('caja.index')" :active="request()->routeIs('caja.*') || request()->routeIs('pedidos.cobrar*')">
                            {{ __('Caja') }}
                        </x-nav-link>
                    @endif
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if (Auth::user()->isAdmin() || Auth::user()->isCocina())
                <x-responsive-nav-link :href="route('cocina.index')" :active="request()->routeIs('cocina.*')">
                    {{ __('Cocina') }}
                </x-responsive-nav-link>
            @endif
